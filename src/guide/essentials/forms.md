...
- `<select>` utiliza la propiedad `value` y el evento `change`.
...
<label for="checkbox">{{ checked ? 'Verdadero' : 'Falso' }}</label>
```

<div class="demo">
  <input type="checkbox" id="checkbox-demo" v-model="checked" />
  <label for="checkbox-demo">{{ checked ? 'Verdadero' : 'Falso' }}</label>
</div>
...
<select v-model="selected">
  <option disabled value="">Por favor, elige uno</option>
  <option>A</option>
  <option>B</option>
  <option>C</option>
</select>
...
Los tipos de entrada incorporados en HTML no siempre satisfacen tus necesidades. Afortunadamente, los componentes de Vue te permiten construir entradas reutilizables con un comportamiento completamente personalizado. ¡Estas entradas incluso funcionan con `v-model`! Para saber más, lee sobre [Uso con `v-model`](/guide/components/events.html#uso-con-v-model) en la guía de Componentes.
