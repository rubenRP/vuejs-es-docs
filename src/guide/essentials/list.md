...
Dentro del ámbito `v-for`, las expresiones del template tienen acceso a todas las propiedades del ámbito padre. Además, `v-for` también admite un segundo alias opcional para el índice del elemento actual:
...
Fíjate en que el valor `v-for` coincide con la firma de la función del callback `forEach`. De hecho, puedes utilizar la desestructuración en el alias del elemento `v-for` de forma similar a la desestructuración de los argumentos de la función:
...
También puedes usar `v-for` para iterar por las propiedades de un objeto.
...
:::tip Nota
Cuando se itera sobre un objeto, el orden se basa en el orden de enumeración de `Object.keys()`, que no se garantiza que sea consistente en todas las implementaciones del motor JavaScript.
:::

## `v-for` con un Rango

`v-for` también puede tomar un entero. En este caso se repetirá el template tantas veces, basado en un rango de `1...n`.
...
:::warning Nota
No es recomendable usar `v-if` y `v-for` en el mismo elemento debido a la precedencia implícita. Consulta la [guía de estilo](/style-guide/rules-essential.html#avoid-v-if-with-v-for) para más detalles.
:::
