...
export default {
  ...
  // `mounted` es un hook del ciclo de vida que explicaremos más adelante
  mounted() {
...
export default {
  // `setup` es un hook especial dedicaco a la composition API.
  setup() {
    const state = reactive({ count: 0 })

    // expone el estado a la template
    return {
      state
    }
  }
...
Las importaciones de nivel superior y las variables declaradas en el `<script setup>` se pueden usar automáticamente en la template del mismo componente.
...
Al igual que todas las demás propiedades de la instancia del componente, se puede acceder a los `methods` desde la template del componente. Dentro de una template, se usan más comúnmente como escuchadores de eventos:
...
Cuando mutas el estado reactivo, el DOM se actualiza automáticamente. Sin embargo, debes tener en cuenta que las actualizaciones de DOM no se aplican de forma sincrónica. En su lugar, Vue los almacena en el búfer hasta el "siguiente tick" en el ciclo de actualización para garantizar que cada componente necesite actualizarse solo una vez, sin importar cuántos cambios de estado hayas realizado.
...
2. Dado que el seguimiento de la reactividad de Vue funciona sobre el acceso a la propiedad, siempre debemos mantener la misma referencia al objeto reactivo. Esto significa que no podemos "reemplazar" fácilmente un objeto reactivo:
   ...
   // la función recibe un número simple y
   // no será capaz de realizar un seguimiento de los cambios en state.count
...
Ten en cuenta que el desempaquetado sólo se aplica si la ref es una propiedad de nivel superior en el contexto del renderizado de la template. Por ejemplo, `foo` es una propiedad de nivel superior, pero `objeto.foo` no lo es.
...
En algunos casos, es posible que necesitemos crear dinámicamente una función de método, por ejemplo, creando un manejador de eventos desbordado:
...
Sin embargo, este enfoque es problemático para los componentes que se reutilizan porque una función de rebote es **de estado**: mantiene algún estado interno sobre el tiempo transcurrido. Si varias instancias de un componente comparten la misma función de rebote, interferirán entre sí.
